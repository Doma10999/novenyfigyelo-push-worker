  ...
  async fetch(request, env) {
    try {
      ...
      if (request.method === 'GET' && path === '/vapid-public-key') {
        ...
        return json(request, env, 200, { publicKey: env.VAPID_PUBLIC_KEY });
      }
      ...
      if (request.method === 'POST' && path === '/subscribe') {
        ...
        const existingIndex = list.findIndex((x) => x.endpoint === entry.endpoint);
        if (existingIndex >= 0) {
      ...
      if (request.method === 'POST' && path === '/unsubscribe') {
        ...
        if (next.length) await env.PUSH_SUBS.put(key, JSON.stringify(next));
      ...
      if (request.method === 'POST' && path === '/send') {
        ...
        const payload = {
          title,
          body: message,
          url: safeAppUrl(body.url || env.APP_URL || 'https://novenyfigyelo.netlify.app/'),
          icon: safeAppUrl(body.icon || env.PUSH_ICON_URL || 'https://novenyfigyelo.netlify.app/icon2.png'),
          badge: safeAppUrl(body.badge || env.PUSH_BADGE_URL || 'https://novenyfigyelo.netlify.app/icon2.png'),
...
async function readSubscriptions(env, key) {
  ...
  try {
    ...
    return Array.isArray(parsed) ? parsed.filter((x) => x?.endpoint && x?.keys) : [];
  } catch {
    return [];
  }
}
...
function safeAppUrl(value) {
  ...
  try {
    ...
    if (url.protocol !== 'https:') return 'https://novenyfigyelo.netlify.app/';
    return url.href;
  } catch {
...
function allowedOrigins(env) {
  return String(env.ALLOWED_ORIGINS || 'https://novenyfigyelo.netlify.app')
    .split(',')
    .map((x) => x.trim())
    .filter(Boolean);
}
